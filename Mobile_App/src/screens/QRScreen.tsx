...
import { Button, Snackbar, IconButton, Divider, Chip } from 'react-native-paper';
  ...
  const sharePayload = async () => {
    try {
      await Share.share({
        ...
        message: `Mi QR de acceso a OKO VISION:\nUsuario: ${user?.username}\nCorreo: ${user?.email}`,
      });
    } catch (e: any) {
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <GlassCard neon style={styles.qrCard}>
            <View style={styles.qrHeader}>
              <View style={styles.qrLogo}>
                <IconButton icon="eye-outline" size={32} iconColor={OKO_COLORS.accentCyan} style={{ margin: 0 }} />
              </View>
              <View>
            ...
            <View style={styles.validity}>
              <Chip mode="flat" icon="clock-check" textStyle={{ color: OKO_COLORS.success, fontSize: 11 }} style={{ backgroundColor: 'rgba(0,255,135,0.08)' }}>
        ...
        <GlassCard style={styles.tipCard} padding={16}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
            <IconButton icon="lightbulb-on-outline" size={22} iconColor={OKO_COLORS.warning} style={{ margin: 0, backgroundColor: 'rgba(255,165,0,0.08)' }} />
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.tipText}>
                ...
                · Si el QR no es reconocido, acércate más al lector.
              </Text>
            </View>
          </View>
        </GlassCard>
...
const styles = StyleSheet.create({
  ...
  subtitle: { color: OKO_COLORS.textSecondary, marginTop: 4, lineHeight: 20 },
  qrCard: { padding: 22 },
  qrHeader: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  qrLogo: {
    width: 54, height: 54, borderRadius: 16,
    backgroundColor: 'rgba(0,242,255,0.12)', alignItems: 'center', justifyContent: 'center',
  },
  qrBrand: { color: OKO_COLORS.accentCyan, fontSize: 22, fontWeight: '900', letterSpacing: 1 },
  qrBrandSub: { color: OKO_COLORS.textSecondary, fontSize: 12, marginTop: -2 },
  qrWrap: { alignItems: 'center', paddingVertical: 6 },
  qrBox: {
    padding: 16, backgroundColor: OKO_COLORS.textPrimary,
    borderRadius: 22, shadowColor: OKO_COLORS.accentCyan, shadowOpacity: 0.25, shadowRadius: 18,
  },
  qrInfo: { gap: 10 },
  ...
  infoValue: { color: OKO_COLORS.textPrimary, fontWeight: '800', fontSize: 13, maxWidth: '65%' },
  ...
  scanBtn: { marginTop: 20, borderRadius: 14, borderWidth: 1, borderColor: OKO_COLORS.border },
