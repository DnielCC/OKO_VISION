...
<div class="card max-w-4xl mx-auto">
    <form action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select class="input-field w-full @error('sexo') border-red-500 @enderror" id="sexo" name="sexo">
                        ...
                        <option value="H" class="bg-gray-900" {{ old('sexo') == 'H' ? 'selected' : '' }}>Hombre (H)</option>
                        <option value="M" class="bg-gray-900" {{ old('sexo') == 'M' ? 'selected' : '' }}>Mujer (M)</option>
                    </select>
                    @error('sexo')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="identificador" class="block text-sm font-medium text-gray-300 mb-2">Matrícula o Pase <span class="text-red-400">*</span></label>
                    <input type="text" class="input-field w-full font-mono @error('identificador') border-red-500 @enderror" 
                           id="identificador" name="identificador" value="{{ old('identificador') }}" required maxlength="9" placeholder="Max. 9 números (Ej. 123456789)">
                    @error('identificador')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <select class="input-field w-full @error('id_rol') border-red-500 @enderror" id="id_rol" name="id_rol" required>
                        ...
                        <option value="1" class="bg-gray-900" {{ old('id_rol', request('role')) == '1' ? 'selected' : '' }}>Administrador</option>
                        <option value="2" class="bg-gray-900" {{ old('id_rol', request('role')) == '2' ? 'selected' : '' }}>Usuario Operador</option>
                        <option value="3" class="bg-gray-900" {{ old('id_rol', request('role')) == '3' ? 'selected' : '' }}>Visitante</option>
                    </select>
                    @error('id_rol')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
